...
wine_attribute <- function(attribute, name = '', scaleYlog10 = TRUE) {
...
hist.chlorides <- ggplot(data = wines) +
    geom_histogram(aes(x=chlorides, fill=as.factor(quality))) +
    scale_fill_brewer(type='div') + 
    scale_y_log10()

outliers.chlorides <- ggplot(wines, aes(x = 1, y = chlorides)) + 
    geom_jitter(alpha = 0.1 ) +
    geom_boxplot(alpha = 0.2, color = 'red') + 
    labs(x ='distance from mean')
    

ggarrange(outliers.chlorides, hist.chlorides,ncol=2, widths = c(1,2))



wine_attribute(wines$chlorides)
```
...
hist.sugar <- ggplot(data = wines) +
    geom_histogram(aes(x=residual.sugar, fill=as.factor(quality))) +
    scale_fill_brewer(type='div') + 
    scale_y_log10()

ggarrange(ggplot(wines, aes(x = 1, y = residual.sugar)) + 
               geom_jitter(alpha = 0.1 ) +
               geom_boxplot(alpha = 0.2, color = 'red' ),
             hist.sugar,ncol=2, widths = c(1,2))
```
...
find out important and the previous analysis. To provide an overview of the 
correlation among variables, I will first plot the scatterplot pairs of the 
attributes we may be interested in:
...
obtained in the previous section. In this way, in this section I intend to build
a linear regression model that could allow us to estimate the wine quality from 
...
measure of 34%. Since the objective of this project was limited on EDA, I will 
...
annotate_figure(plotOne,
               top = text_grob("Wine acidities analysis", 
                               face = "bold", size = 14),
