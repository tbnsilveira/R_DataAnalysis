...
many quality levels are assigned to 11% of alcohol, the relevant correlation 
among alcohol and quality arises only when we are dealing with the bordering
values.  

To check it out, I will build a table with the correlation among quality and 
alcohol for each one of the quality values:

```{r}
## values whose quality is higher than 7 together:
alcQuality <- group_by(wines, quality)
alcQuality_corr <- summarise(alcQuality,
                             alcohol_mean = mean(alcohol),
                             alcohol_sd = sd(alcohol),
                             )#quality_corr = cor.test(alcohol, quality, method = 'pearson'))
# best <- group_by(subset(wines, quality >= 7), quality)
acidities <- summarise(bestAcid, 
          fixed_min = min(fixed.acidity),
          fixed_mean = mean(fixed.acidity),
